...
```bash
  ...
  # Apple M1
  docker run --cap-add SYS_PTRACE -e 'ACCEPT_EULA=1' -e 'MSSQL_SA_PASSWORD=B9037!m8947#' -e 'MSSQL_PID=Developer' -e 'TZ=Asia/Seoul' -e 'MSSQL_COLLATION=Korean_Wansung_CI_AS' -p 59273:1433 -v '/Users/vivakr/Docker/SQLServer:/var/opt/mssql/data' --name viv-sql --hostname viv-sql -d --restart unless-stopped mcr.microsoft.com/azure-sql-edge
